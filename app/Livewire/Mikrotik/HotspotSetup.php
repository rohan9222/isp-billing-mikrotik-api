...
class HotspotSetup extends Component
{
    ...
    {
        try { $this->sessions = app(MikrotikController::class)->getHotspotActiveSessions($this->selectedRouter); }
        catch (\Exception $e) { flash()->error($e->getMessage()); }
    }

    private function resetData(): void { $this->servers = $this->profiles = $this->users = $this->userProfiles = $this->sessions = []; }
