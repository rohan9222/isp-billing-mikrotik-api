...
<div class="form-group {{ $column ?? 'input-group-sm col-md-6 col-sm-12' }}" style="{{ $groupstyle ?? '' }}">
    ...
    @elseif ($type == 'mobile')
        <div class="input-group input-group-sm">
            ...
            <input type="number" id="{{ $name }}" class="form-control @error($name) is-invalid @enderror" placeholder="{{ __($placeholder ?? $label) }}" name="{{ $name }}" wire:model="{{$name}}" maxlength="11" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" pattern="[0-9]{11}">
            <x-error name='{{ $name }}' />
        </div>
